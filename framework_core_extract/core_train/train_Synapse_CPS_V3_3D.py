...
try:
    ...
    class transforms:  # type: ignore[override]
        Compose = _Compose


parser = argparse.ArgumentParser()
parser.add_argument("--dataset_name", type=str, default="Synapse")
parser.add_argument("--root_path", type=str, default="./data/Synapse/")
...
parser.add_argument("--labelnum", type=int, default=4)
...
parser.add_argument("--aux_export_debug_dir", type=str, default="")
parser.add_argument("--aux_export_every", type=int, default=0)
parser.add_argument("--aux_export_case_limit", type=int, default=1)
...
from dataloaders.dataset import RandomCrop, Synapse_fast, ToTensor, TwoStreamBatchSampler
...
class FeatureVNet(VNet):
    """
    Local wrapper that exposes decoder feature x9 without editing the reference GA tree.
    """
...
def downsample_feat_3d(feat_3d, token_cube_size):
    target_shape = (token_cube_size, token_cube_size, token_cube_size)
    return F.adaptive_avg_pool3d(feat_3d, output_size=target_shape)
...
def export_aux_debug_npz(debug_dir, iter_num, epoch_num, payload, feat_tokens, label_batch, labeled_bs, case_limit):
    if not debug_dir or payload is None:
        return
    os.makedirs(debug_dir, exist_ok=True)
    limit = max(1, int(case_limit))

    export_dict = {
        "iter_num": np.asarray([iter_num], dtype=np.int32),
        "epoch_num": np.asarray([epoch_num], dtype=np.int32),
        "feat_tokens": feat_tokens[:limit].detach().cpu().numpy().astype(np.float32),
        "label_batch": label_batch[:limit].detach().cpu().numpy().astype(np.int16),
        "aux_stats_json": np.asarray([str(payload.get("aux_stats", {}))], dtype=object),
    }
    ...
    tensor_keys = [
        ...
        "tail_target_map",
        "tail_entry_mask_map",
        "tail_entry_strict_map",
    ]
    for key in tensor_keys:
        value = payload.get(key)
        if value is None:
            continue
        export_dict[key] = value[:limit].numpy()

    for key in ["gt_center_map", "center_weight_maps", "hard_center_map"]:
        value = payload.get(key) or {}
        if not value:
            continue
        for class_id, tensor in value.items():
            export_dict[f"{key}_class{int(class_id)}"] = tensor.numpy()

    out_path = os.path.join(debug_dir, f"aux_debug_iter_{iter_num:05d}.npz")
...
if args.labelnum == 2:
    ...
    raise ValueError("Unsupported labelnum")
...
def train(labeled_list, unlabeled_list, eval_list, fold_id=1):
    ...
    logging.info(
        "Resume state: start_iter=%d, best_dice=%.4f, start_epoch=%d",
        iter_num,
        best_dice_avg,
        start_epoch,
    )
    ...
    for epoch_num in iterator:
        ...
        for _, sampled_batch in enumerate(trainloader):
            ...
            if args.aux_enable and aux_module is not None and args.labeled_bs > 0:
                ...
                y_dist_flat = mask3d_to_token_dist(
                    label_l,
                    num_classes=num_classes,
                    token_shape=feat_3d_ds.shape[2:],
                )
                x_class_tok, y_class_idx = build_class_tokens_from_token_dist(
                    feat_tokens,
                    y_dist_flat,
                )
                (
            ...
            if iter_num % 500 == 0:
                ...
                model_A.eval()
                model_B.eval()
                dice_all, std_all, metric_all_cases = test_util_vnet_AB.validation_all_case_fast(
                    model_A,
                    model_B,
                    num_classes=num_classes,
                    base_dir=args.root_path,
                ...
                if dice_avg > best_dice_avg:
                    ...
                    torch.save(model_A.state_dict(), os.path.join(snapshot_path, f"iter_{str(iter_num).zfill(5)}_dice_{str(best_dice_avg)[:8]}_best_A.pth"))
                    torch.save(model_B.state_dict(), os.path.join(snapshot_path, f"iter_{str(iter_num).zfill(5)}_dice_{str(best_dice_avg)[:8]}_best_B.pth"))
                    if aux_module is not None:
                        torch.save(aux_module.state_dict(), os.path.join(snapshot_path, f"iter_{str(iter_num).zfill(5)}_dice_{str(best_dice_avg)[:8]}_best_AUX.pth"))
                model_A.train()
                model_B.train()
            ...
            if (
                args.aux_enable
                and aux_module is not None
                and args.aux_export_every > 0
                and iter_num % args.aux_export_every == 0
            ):
                export_aux_debug_npz(
                    debug_dir=args.aux_export_debug_dir,
                    iter_num=iter_num,
                    epoch_num=epoch_num,
                    payload=getattr(aux_module, "last_aux_debug_payload", None),
                    feat_tokens=feat_tokens,
                    label_batch=label_l,
                    labeled_bs=args.labeled_bs,
                    case_limit=args.aux_export_case_limit,
                )
